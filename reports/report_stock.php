...
>
  <head>
    <?php 
      if ($lang == 1) {
        ...
        $page_title = 'ស្តុកសម្ភារៈ';
        $text_dropdown = 'ពត៌មានបន្ថែម';
        $graph_title = 'របាយការណ៍ស្តុក';
        $th1 = 'លេខកូដ';
        $th2 = 'រូបភាព';
        $th3 = 'ពត៌មានសម្ភារៈ';
        $th4 = 'ចំនួន';
  ...
  <foot>
    ...
    <script src="../script/page_report_stock.js"></script>
    <script type="text/javascript">
      window.onload = function(){
        var activeMenu = document.getElementById("Analytics");
        activeMenu.classList.add("open");
        activeMenu.classList.add("active");
        var activeSubMenu = document.getElementById("Stock Report");
...
<?php
  function getDataByMonth() {
    ...
    foreach($month as $i => $m) {
      $i += 1;
      $year = date("Y");
      $ROW = mysqli_fetch_assoc(mysqli_query($CON, "SELECT SUM(`request_qty` * `prod_unit_cost`) AS `expense` FROM `tbl_stock_request` AS `r` INNER JOIN `tbl_product` AS `p` ON (`r`.`prod_id` = `p`.`id`) WHERE MONTH(`r`.`timestamp`) = '$i' AND YEAR(`r`.`timestamp`) = '$year' AND `request_status` = 2"));
      $total = ($ROW['expense'] == 0) ? 0 : $ROW['expense'];
      $data_by_month .= str_replace(',', '', number_format($total, 0)).', ';
    }
