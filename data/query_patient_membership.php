<?php
  ...
  else if ($qid == 2) { getJSRow ('tbl_membership'); }
  ...
  function fnTable() {
    ...
    if (isset($_POST['search']['value'])) {
      ...
      $SQL .= " WHERE `memb_type` LIKE '%". $search_value ."%'";
    }
    if (isset($_POST['order'])) {
    	$column_name = $_POST['order'][0]['column'];
      $order = $_POST['order'][0]['dir'];
      $SQL .= " ORDER BY ".$columns[$column_name]." ".$order."";
    } else { $SQL .= " ORDER BY `memb_discount` ASC"; }
    if ($_POST['length'] != -1) {
      $start = $_POST['start'];
      $length = $_POST['length'];
    ...
    while ($ROW = mysqli_fetch_assoc($QUERY)) {
      ...
      $sub_array[] = $ROW['memb_type'];
      $sub_array[] = '<center>'.$ROW['memb_discount'].' %</center>';
  ...
  function fnInsert() {
    ...
    $SQL = "INSERT INTO `tbl_membership` (`memb_type`, `memb_discount`) VALUES ('$membership', '$discount')";
    $QUERY = mysqli_query($CON, $SQL);
    $data = ($QUERY == TRUE) ? array('status' => 'True') : array('status' => 'False');
    echo json_encode($data);
  }
  ...
  function fnUpdate() {
    ...
    $discount = $_POST['discount'];
    // $SQL = "INSERT INTO `tbl_membership` (`memb_type`, `memb_discount`) VALUES ('$membership', '$discount')";
    $SQL = "UPDATE `tbl_membership` SET `memb_type` = '$membership', `memb_discount` = '$discount' WHERE `id` = '$id'";
    $QUERY = mysqli_query($CON, $SQL);
    $data = ($QUERY == TRUE) ? array('status' => 'True') : array('status' => 'False');
    echo json_encode($data);
  }
