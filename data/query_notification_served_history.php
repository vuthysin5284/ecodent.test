<?php
  ...
  function fnTable() {
    ...
    $SQL = "SELECT `sv`.`id`, DATE_ADD(`sv`.`timestamp`, INTERVAL 10 HOUR) as `timestamp`, `sv`.`cust_id`, `cust_code`, `cust_image`, `cust_fname`, `cust_gender`, `cust_dob`, `cust_contact`, `cust_address`, `queue_id`,  `queue_duration`, `staff_fname`, `serve_note`, `trmt_duration`, `serve_status` FROM `tbl_serving` AS `sv` INNER JOIN `tbl_customer` AS `c` ON (`sv`.`cust_id` = `c`.`id`) INNER JOIN `tbl_staff` AS `s` ON (`sv`.`staff_id` = `s`.`id`) INNER JOIN `tbl_queue` AS `q` ON (`sv`.`queue_id` = `q`.`id`) INNER JOIN `tbl_treatment_duration` AS `d` ON (`q`.`queue_duration` = `d`.`id`) WHERE (`sv`.`timestamp` BETWEEN '$start' AND '$end') AND `serve_status` = 2";
    ...
    } else { $SQL .= " ORDER BY `sv`.`timestamp` ASC"; }
    if ($_POST['length'] != -1) {
      $start = $_POST['start'];
      $length = $_POST['length'];
    ...
    while ($ROW = mysqli_fetch_assoc($QUERY)) {
      ...
      $btnShow = '<a href="patient_treatment_plan.php?pgid=7&cid='.$ROW['cust_code'].'" class="btn btn-icon btn-primary showBtn" data-bs-toggle="tooltip" data-bs-placement="top" title="View"><span class="tf-icons bx bx-show"></span></a>';
