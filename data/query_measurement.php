<?php
  ...
  else if ($qid == 2) { getJSRow ('tbl_measurement'); }
  ...
  function fnDelete() {
    global $CON;
    $id = $_POST['id'];
    $SQL = "DELETE FROM `tbl_measurement` WHERE id = '$id'";
  ...
  function fnTable() {
    global $CON;
    $SQL = "SELECT * FROM `tbl_measurement`";
    $output = array();
    $columns = array(
      0 => 'id',
      1 => 'measure_eng',
    ...
    if (isset($_POST['search']['value'])) {
      ...
      $SQL .= " WHERE `measure_eng` LIKE '%". $search_value ."%'";
    }
    ...
    } else { $SQL .= " ORDER BY `measure_eng` ASC"; }
    if ($_POST['length'] != -1) {
      $start = $_POST['start'];
      $length = $_POST['length'];
    ...
    while ($ROW = mysqli_fetch_assoc($QUERY)) {      
      ...
      $sub_array[] = '<center>'.($i += 1).'</center>';
			$sub_array[] = '<center>'.$ROW['measure_eng'].'</center>';
      $sub_array[] = '<center>'.$ROW['measure_kh'].'</center>';
  ...
  function fnInsert() {
    global $CON;
		$id = $_POST['id'];
    $measure_eng = $_POST['measure_eng'];
    $measure_kh = $_POST['measure_kh'];
    $SQL = "INSERT INTO `tbl_measurement` (`measure_eng`, `measure_kh`) VALUES ('$measure_eng', '$measure_kh')";
    $QUERY = mysqli_query($CON, $SQL);
  ...
  function fnUpdate() {
    ...
    $measure_eng = $_POST['measure_eng'];
    $measure_kh = $_POST['measure_kh'];
    $SQL = "UPDATE `tbl_measurement` SET `measure_eng` = '$measure_eng', `measure_kh` = '$measure_kh' WHERE `id` = '$id'";
    $QUERY = mysqli_query($CON, $SQL);
    $data = ($QUERY == TRUE) ? array('status' => 'true') : array('status' => 'false');
    echo json_encode($data);
	}
  ...
  function getLastId() {
    global $CON;
		$SQL = "SELECT `id` FROM `tbl_measurement` ORDER BY `id` DESC";
    $QUERY = mysqli_query($CON, $SQL);
